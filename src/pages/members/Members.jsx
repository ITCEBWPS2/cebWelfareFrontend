...
const MembersTable = () => {
  ...
  const handleUpdate = async (memberId, updatedData) => {
    try {
      ...
      setMembers(members.map(member => member._id === editMemberId ? response.data : member));
        ...
        <div className="overflow-x-auto">
          <table className="w-full bg-white rounded-lg shadow-lg">
            <thead className="bg-red-900 text-white">
              <tr>
                <th className="px-6 py-3 text-left text-sm font-semibold">EPF no</th>
                <th className="px-6 py-3 text-left text-sm font-semibold">Welfare no</th>
                <th className="px-6 py-3 text-left text-sm font-semibold">Name</th>
                <th className="px-6 py-3 text-left text-sm font-semibold">Date of Birth</th>
                <th className="px-6 py-3 text-left text-sm font-semibold">Date of Registered</th>
                <th className="px-6 py-3 text-left text-sm font-semibold">Date of Joined</th>
                <th className="px-6 py-3 text-left text-sm font-semibold">Payroll</th>
                <th className="px-6 py-3 text-left text-sm font-semibold">Status</th>
                <th className="px-6 py-3 text-left text-sm font-semibold">Contact Number</th>
                <th className="px-6 py-3 text-left text-sm font-semibold">Whatsapp Number</th>
            ...
            <tbody>
              {members.length === 0 ? (
                <tr>
                  <td className="border px-6 py-4 text-center" colSpan="11">No members found</td>
                </tr>
              ) : (
                members.map((member, index) => (
                  <tr key={member._id} className={`${index % 2 === 0 ? 'bg-yellow-50' : 'bg-red-50'}`}>
                    {editMemberId === member._id ? (
                      <>
                        ...
                        <td className="border px-6 py-4 flex justify-center space-x-2">
                          <button
                            onClick={() => handleUpdate(member._id, editedMember)}
                      ...
                      <>
                        ...
                        <td className="border px-6 py-4">{member.dateOfRegistered}</td>
                        ...
                        <td className="border px-6 py-4">{member.contactNo?.number}</td>
                        <td className="border px-6 py-4">{member.contactNo?.whatsappNo}</td>
                        <td className="border px-6 py-4 flex justify-center space-x-2">
                          <button
