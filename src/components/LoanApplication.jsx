...
const LoanApplication = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "epf") {
      ...
      // Automatically populate name if EPF matches a member
      // const matchedMember = members.find((member) => member.epf === value);
      const matchedMember = members.find(
        (member) => String(member.epf).trim() === String(value).trim()
      );

      console.log("Members:", members);
      console.log("EPF to match:", formData.epf);
      console.log("Matched member:", matchedMember);

      if (matchedMember) {
        setFormData((prev) => ({ ...prev, name: matchedMember.name }));
        setShowSuggestions(false); // Hide suggestions if exact match is found
      }
    } else {
      setFormData((prev) => ({ ...prev, [name]: value }));
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Validate EPF Number
    const isEPFValid = members.some((member) => member.epf === formData.epf);

    if (!isEPFValid) {
      toast.error("Invalid EPF number. Please enter a valid EPF.");
    ...
    if (monthDifference < 10) {
      const confirmation = window.confirm(
        `Warning: The person is going to retire on ${retirementDate.toDateString()}, which is less than 10 months from today. Do you want to proceed?`
      );
      if (!confirmation) {
    ...
    const dataToSubmit = {
      ...
      retirementDate: retirementDate.toISOString().split("T")[0], // Add retirement date to submitted data
    };
    ...
    <form onSubmit={handleSubmit} className="p-8 space-y-12">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="relative">
          <Link to={`/dashboard/members/${members.data}/loans`}>
